...
export class CircleService {
  ...
  async killVictimInRound(round: IRound) {
    ...
    try {
      let res: IPlayer = await this.http
        .post<IPlayer>(
          `/api/users/${user}/circle/${round.circleName}/killVictim`,
          { headers }
        )
        .toPromise();
  ...
  async getAllCircles() {
    ...
    let result: Promise<Boolean>;
    let res: ICircle[] = await this.http
      .get<ICircle[]>("/api/circles", { headers })
      .toPromise();
  ...
  async joinCircle(circleName: string) {
    ...
    try {
      let res: ICircle[] = await this.http
        .post<ICircle[]>(`/api/circles/${circleName}/join`, { headers })
        .toPromise();
    } finally {
  ...
  async activateCircle(circleName: string) {
    ...
    try {
      let res: ICircle[] = await this.http
        .post<ICircle[]>(`/api/circles/${circleName}/activate`, { headers })
        .toPromise();
    } finally {
      this.userService.loginFromStored();
    }
  ...
  async createNewCircle(circleName: string) {
    ...
    try {
      let res: ICircle[] = await this.http
        .post<ICircle[]>(`/api/circles/${circleName}`, { headers })
        .toPromise();
    } catch {}
  }
}
